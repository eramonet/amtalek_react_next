...
export default function ComboBoxTwo({
  width = "w-full",
  setValue,
  // setDataSearch,
  stateName,
  ...
  useEffect(() => {
    if (data && formData) {
      const selectedId = formData.getDefaultValueFromURL;
      const selectedOption = data.find(
        // (location: any) => Number(location.id) === Number(selectedId)
        (location: any) => Number(location.id) === Number(formData[stateName])
      );
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data, getDefaultValueFromURL, formData]);

  const handleChange = (selectedId: any) => {
    setSelectedLocationId(selectedId || formData[stateName]);

    setFormData((prevFormData: any) => ({
      ...prevFormData,
      [stateName]: selectedId, // تأكد من استخدام stateName لتحديد المفتاح المناسب
    }));
    ...
    if (typeof setValue === "function") {
      setValue(stateName, selectedId || formData[stateName]);
    }
            ...
            <CommandGroup>
              {options.map((option: any) => (
                <CommandItem
                  ...
                  key={option.value}
                  value={option.label || formData?.getDefaultValueFromURL}
                  onSelect={() => {
