...
export async function generateMetadata({ params: { locale, title } }: any) {
  const i18nNamespaces = ["Pages_SearchProperty", "Pages_LandingPage"];

  const { t } = await initTranslations(locale, i18nNamespaces);
  const decodedName = decodeURIComponent(title);
  return {
    title: t("tab.title")+ `| ${decodedName.replace(/\-/g, " ")}`,
    description: decodedName, //t("tab.description"),
    icons: {
      icon: "/fav-icon.png",
    },
  };
